...
$tileServers = [
    ...
    // Add more tile servers here if needed
];
...
$tileServer = $_GET['srv'] ?? 'default';
if($tileServer == 'default') $tileServer = 'osm_de';


$storage = $base_storage . $tileServer . '/';
$tileUrl = $tileServers[$tileServer] ?? 'https://tile.openstreetmap.org/{z}/{x}/{y}.png';

/**
 * Function to Download Tiles
 */

function download($storage, $tileUrl, $z, $x, $y, $operator) {
    // Set maximum execution time to unlimited
    set_time_limit(0);
    // Define the source URL for the tile
    $source = str_replace(['{z}', '{x}', '{y}'], [$z, $x, $y], $tileUrl);
...
if (file_exists($tilePath)) {
    ...
    if (time() >= ($age + $ttl)) {
        download($storage, $tileUrl, $z, $x, $y, $operator);
    }
} else {    
